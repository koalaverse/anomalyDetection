test_that("tabulate_state_vector output has correct dimensions", {
  ...
  expect_equal(tabulate_state_vector(security_logs, 30) %>% dim(), c(10, 36))
  expect_equal(tabulate_state_vector(security_logs, 5, 4, 2) %>% dim(), c(60, 18))
...
test_that("tabulate_state_vector computes correctly", {
  ...
  expect_equal(tabulate_state_vector(security_logs, 30, 4, 2) %>% .[[4, 4]], 14)
